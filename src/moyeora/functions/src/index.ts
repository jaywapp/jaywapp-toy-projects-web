...
function hasPermissionInMemberData(
...
function isActiveMemberData(
  memberData: FirebaseFirestore.DocumentData | undefined,
): memberData is FirebaseFirestore.DocumentData {
  return !!memberData && memberData.status === "active";
}

function canManageMembersOrEvents(memberData: FirebaseFirestore.DocumentData): boolean {
  return hasPermissionInMemberData(memberData, "member.manage") ||
    hasPermissionInMemberData(memberData, "event.manage");
}

function hasPermissionFromClaims(
  authToken: unknown,
  groupId: string,
  permission: string,
): boolean {
  if (!authToken || typeof authToken !== "object") return false;
  const tokenMap = authToken as Record<string, unknown>;
  const moyeora = tokenMap.moyeora;
  if (!moyeora || typeof moyeora !== "object") return false;
  const groups = (moyeora as Record<string, unknown>).groups;
  if (!groups || typeof groups !== "object") return false;
  const groupClaims = (groups as Record<string, unknown>)[groupId];
  if (!groupClaims || typeof groupClaims !== "object") return false;
  const role = (groupClaims as Record<string, unknown>).role;
  if (role === "owner") return true;
  const perms = (groupClaims as Record<string, unknown>).perms;
...
async function upsertGroupClaims(
  userId: string,
  groupId: string,
  role: string,
  permissions: string[],
  ...
  const existingMoyeora = (existingClaims.moyeora ?? {}) as Record<string, unknown>;
  const existingGroups = (existingMoyeora.groups ?? {}) as Record<string, unknown>;
...
async function removeGroupClaims(
  userId: string,
  groupId: string,
): Promise<boolean> {
  try {
    ...
    logger.warn("removeGroupClaims failed", {userId, groupId, error});
    return false;
  }
}
...
function compactPermissions(permissions: unknown): string[] {
...
// GITHUB_ISSUES_TOKEN: Blaze 플랜 없이 Secret Manager 사용 불가하므로 .env로 대체
type RecurrenceRule = "weekly" | "biweekly" | "monthly";

function isAutomationFeatureEnabled(flagName: string): boolean {
...
function computeNextRecurringStartAt(source: Date, rule: RecurrenceRule): Date {
  if (rule === "weekly") {
...
function recurrenceLookAheadMillis(rule: RecurrenceRule): number {
  switch (rule) {
    case "weekly":
...
function daysUntilDueInKst(dueDate: Date, nowDate: Date): number {
...
function normalizeSentHours(rawValue: unknown): number[] {
...
function parseGithubIssueLabels(rawValue: string | undefined): string[] {
...
function toIsoStringIfTimestamp(raw: unknown): string | null {
...
async function getActiveMembers(groupId: string): Promise<MemberInfo[]> {
...
async function getMemberTokens(groupId: string, uid: string): Promise<string[]> {
...
async function getNotificationSettings(groupId: string, uid: string): Promise<NotificationSettings> {
...
async function sendMulticastNotification(
  ...
  try {
    ...
    if (result.failureCount > 0) {
      logger.warn("Multicast partial failure", {
        successCount: result.successCount,
        failureCount: result.failureCount,
      });
    }
  } catch (error) {
    logger.error("Multicast send failed", error);
  }
}

async function getMembersWithoutResponse(groupId: string, eventId: string): Promise<string[]> {
...
export const createInviteCode = onCall(async (request) => {
  ...
  const expiresInDays =
    typeof expiresInDaysRaw === "number" ? Math.floor(expiresInDaysRaw) : 7;
  const maxUses = typeof maxUsesRaw === "number" ? Math.floor(maxUsesRaw) : 10;
  if (expiresInDays < 1 || expiresInDays > 30) {
    throw new HttpsError("invalid-argument", "expiresInDays는 1~30일이어야 합니다.");
  }
  if (maxUses < 1 || maxUses > 200) {
    throw new HttpsError("invalid-argument", "maxUses는 1~200이어야 합니다.");
  }
  ...
  let createdExpiresAt: admin.firestore.Timestamp | null = null;
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    try {
      ...
      logger.error("createInviteCode failed", {groupId, callerUid, error});
      throw new HttpsError("internal", "초대코드 발급 중 오류가 발생했습니다.");
    }
  }

  if (!createdCode || !createdExpiresAt) {
    throw new HttpsError("aborted", "초대코드를 발급하지 못했습니다. 잠시 후 다시 시도해 주세요.");
  }
...
export const revokeInviteCode = onCall(async (request) => {
  const callerUid = request.auth?.uid;
  if (!callerUid) {
    throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
  }
  ...
  if (!groupId || code.length < 6) {
    throw new HttpsError("invalid-argument", "groupId와 code가 필요합니다.");
  }
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!inviteSnap.exists && !codeSnap.exists) {
      throw new HttpsError("not-found", "초대코드를 찾을 수 없습니다.");
    }

    const inviteData = inviteSnap.data() ?? codeSnap.data() ?? {};
    if (inviteData.groupId !== groupId) {
      throw new HttpsError("failed-precondition", "해당 그룹의 초대코드가 아닙니다.");
    }
...
export const requestJoinWithInviteCode = onCall(async (request) => {
  const callerUid = request.auth?.uid;
  if (!callerUid) {
    throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
  }
  ...
  const code = normalizeInviteCode(rawCode ?? "");
  if (code.length < 6) {
    throw new HttpsError("invalid-argument", "유효한 초대코드를 입력해 주세요.");
  }
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    if (!inviteCodeSnap.exists) {
      throw new HttpsError("not-found", "유효하지 않은 초대코드입니다.");
    }
    const inviteCodeData = inviteCodeSnap.data() ?? {};
    const groupId = inviteCodeData.groupId;
    if (typeof groupId !== "string" || groupId.trim().length === 0) {
      throw new HttpsError("failed-precondition", "초대코드 상태가 올바르지 않습니다.");
    }
    ...
    if (maxUses > 0 && useCount >= maxUses) {
      throw new HttpsError("failed-precondition", "사용 횟수를 초과한 초대코드입니다.");
    }
    ...
    const memberMax =
      limits && typeof limits === "object" && typeof (limits as Record<string, unknown>).memberMax === "number" ?
        (limits as Record<string, number>).memberMax :
        null;
    ...
    if (memberMax !== null && needsMemberCountIncrement && currentMemberCount >= memberMax) {
      throw new HttpsError(
        "failed-precondition",
        `멤버 정원(${memberMax}명)을 초과해 가입할 수 없습니다.`,
      );
    }
  ...
  if (result.status === "joined") {
    try {
      ...
      logger.warn("requestJoinWithInviteCode claims update failed", {
...
export const approveMember = onCall(async (request) => {
  ...
  if (!groupId || !userId) {
    throw new HttpsError("invalid-argument", "groupId와 userId가 필요합니다.");
  }
  ...
  let beforeTarget: Record<string, unknown> | null = null;
  ...
  try {
    ...
    logger.error("approveMember claims update failed", {
      groupId,
      userId,
      error,
    });
...
export const setRoleAndClaims = onCall(async (request) => {
  const callerUid = request.auth?.uid;
  if (!callerUid) {
    throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
  }
  ...
  const rawPermissions = request.data?.permissions as unknown;
  if (!groupId || !userId || !role) {
    throw new HttpsError("invalid-argument", "groupId, userId, role이 필요합니다.");
  }
  if (!["member", "admin", "treasurer"].includes(role)) {
    throw new HttpsError(
      "invalid-argument",
      "role은 member/admin/treasurer만 허용됩니다. 모임장 위임은 별도 기능을 사용해 주세요.",
    );
  }
...
export const delegateGroupOwner = onCall(async (request) => {
  const callerUid = request.auth?.uid;
  if (!callerUid) {
    throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
  }
  ...
  const groupId = request.data?.groupId as string | undefined;
  const newOwnerUid = request.data?.newOwnerUid as string | undefined;
  if (!groupId || !newOwnerUid) {
    throw new HttpsError("invalid-argument", "groupId, newOwnerUid가 필요합니다.");
  }
  ...
  let beforeOwnerId: string | null = null;
  let beforeTargetRole: string | null = null;
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (beforeOwnerId !== callerUid) {
      throw new HttpsError("permission-denied", "현재 모임장만 위임할 수 있습니다.");
    }

    const callerMember = callerMemberSnap.data() ?? {};
    if (callerMember.status !== "active" || callerMember.role !== "owner") {
      throw new HttpsError("permission-denied", "현재 모임장 권한이 확인되지 않았습니다.");
    }

    if (!newOwnerMemberSnap.exists) {
      throw new HttpsError("not-found", "위임 대상 멤버를 찾을 수 없습니다.");
    }
    const newOwnerMember = newOwnerMemberSnap.data() ?? {};
    if (newOwnerMember.status !== "active") {
      throw new HttpsError("failed-precondition", "활성 멤버에게만 위임할 수 있습니다.");
    }
  ...
  try {
    ...
    logger.error("delegateGroupOwner caller claims update failed", {
      groupId,
      callerUid,
      error,
    });
  ...
  try {
    ...
    logger.error("delegateGroupOwner target claims update failed", {
      groupId,
      newOwnerUid,
      error,
    });
...
export const deleteGroup = onCall({timeoutSeconds: 540, memory: "1GiB"}, async (request) => {
  const callerUid = request.auth?.uid;
  if (!callerUid) {
    throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
  }
  ...
  if (ownerId !== callerUid && !callerIsOwnerRole) {
    throw new HttpsError("permission-denied", "모임장만 모임을 삭제할 수 있습니다.");
  }
  ...
  try {
    ...
    logger.error("deleteGroup recursive delete failed", {
      groupId,
      callerUid,
      error,
    });
    throw new HttpsError("internal", "모임 삭제 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.");
  }
...
export const deleteEvent = onCall(async (request) => {
  ...
  if (!groupId || !eventId) {
    throw new HttpsError("invalid-argument", "groupId와 eventId가 필요합니다.");
  }
  ...
  if (isAutomationFeatureEnabled("ENABLE_EVENT_CANCEL_ALERT")) {
    try {
      ...
      const groupName = (groupSnap.data()?.name as string | undefined) ?? "Moyeora";
      ...
      const notified = await Promise.all(targetUids.map(async (uid) => {
        ...
        if (!settings.eventReminderEnabled) return 0;
      ...
      logger.warn("deleteEvent cancellation notify failed", {
        groupId,
        eventId,
        error,
      });
  ...
  try {
    ...
    throw new HttpsError("internal", "일정 삭제 처리 중 오류가 발생했습니다.");
  }
...
export const deleteNotice = onCall(async (request) => {
  const callerUid = request.auth?.uid;
  if (!callerUid) {
    throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
  }
  ...
  const groupId = request.data?.groupId as string | undefined;
  const noticeId = request.data?.noticeId as string | undefined;
  if (!groupId || !noticeId) {
    throw new HttpsError("invalid-argument", "groupId와 noticeId가 필요합니다.");
  }
  ...
  try {
    ...
    logger.error("deleteNotice failed", {
      groupId,
      noticeId,
      callerUid,
      error,
    });
    throw new HttpsError("internal", "공지 삭제 처리 중 오류가 발생했습니다.");
  }
...
export const deletePoll = onCall(async (request) => {
  ...
  const groupId = request.data?.groupId as string | undefined;
  const pollId = request.data?.pollId as string | undefined;
  if (!groupId || !pollId) {
    throw new HttpsError("invalid-argument", "groupId와 pollId가 필요합니다.");
  }
  ...
  try {
    ...
    logger.error("deletePoll failed", {
      groupId,
      pollId,
      callerUid,
      error,
    });
    throw new HttpsError("internal", "투표 삭제 처리 중 오류가 발생했습니다.");
  }
...
export const kickMember = onCall(async (request) => {
  ...
  if (callerUid === userId) {
    throw new HttpsError("failed-precondition", "자기 자신은 강퇴할 수 없습니다.");
  }
  ...
  let beforeTargetRole: string | null = null;
  let beforeTargetStatus: string | null = null;
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!targetMemberSnap.exists) {
      throw new HttpsError("not-found", "대상 멤버를 찾을 수 없습니다.");
    }
    const targetMemberData = targetMemberSnap.data() ?? {};
    if (targetMemberData.status !== "active") {
      throw new HttpsError("failed-precondition", "활성 상태 멤버만 강퇴할 수 있습니다.");
    }
    ...
    if (targetRole === "owner") {
      throw new HttpsError("permission-denied", "모임장은 강퇴할 수 없습니다.");
    }
    if (callerRole !== "owner" && targetRole !== "member") {
      throw new HttpsError("permission-denied", "모임장만 운영진을 강퇴할 수 있습니다.");
    }
...
export const logClientEvent = onCall(async (request) => {
  const uid = request.auth?.uid;
  if (!uid) {
    throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
  }

  const levelRaw = request.data?.level as string | undefined;
  const level = ["debug", "info", "warn", "error"].includes(levelRaw ?? "") ?
    (levelRaw as "debug" | "info" | "warn" | "error") :
    "info";
  const messageRaw = request.data?.message as string | undefined;
  ...
  const context = contextRaw && typeof contextRaw === "object" ? contextRaw as Record<string, unknown> : {};
  ...
  if (groupId) {
    ...
    if (!memberSnap.exists || memberData.status !== "active") {
      throw new HttpsError("permission-denied", "해당 그룹 로그를 기록할 권한이 없습니다.");
    }
  }
  ...
  if (level === "error") {
    logger.error("client-log", payload);
  } else if (level === "warn") {
    logger.warn("client-log", payload);
  } else {
    logger.info("client-log", payload);
  }
...
async function exchangeKakaoAccessToken(accessToken: string): Promise<{
  customToken: string;
  kakaoId: string;
  kakaoProfile: {nickname: string | null; profileImageUrl: string | null};
}> {
  const fetchImpl = (globalThis as {fetch?: typeof fetch}).fetch;
  if (!fetchImpl) {
    logger.error("fetch is unavailable in functions runtime");
    throw new HttpsError("internal", "서버 설정 오류");
  }

  let kakaoMe: KakaoMeResponse;
  try {
    ...
    if (!response.ok) {
      logger.warn("Kakao me API failed", {status: response.status});
      throw new HttpsError("unauthenticated", "카카오 토큰 검증 실패");
    }

    kakaoMe = await response.json() as KakaoMeResponse;
  } catch (error) {
  ...
  if (kakaoIdRaw == null) {
    throw new HttpsError("unauthenticated", "카카오 사용자 식별 실패");
  }
  ...
  try {
    ...
    if (authError.code === "auth/user-not-found") {
      ...
      throw new HttpsError("internal", "사용자 동기화 실패");
    }
  }
...
async function exchangeKakaoAuthCodeToAccessToken(
  code: string,
  redirectUri: string,
): Promise<string> {
  const fetchImpl = (globalThis as {fetch?: typeof fetch}).fetch;
  ...
  if (!restApiKey || restApiKey.trim().length === 0) {
    ...
    throw new HttpsError("failed-precondition", "카카오 서버 설정이 필요합니다.");
  }
  ...
  try {
    ...
    logger.error("Kakao auth code exchange failed", error);
    throw new HttpsError("internal", "카카오 토큰 교환에 실패했습니다.");
  }

  if (!tokenResponse.ok) {
    const failureBody = await tokenResponse.text();
    logger.warn("Kakao token endpoint failed", {
      status: tokenResponse.status,
      body: failureBody,
    });
    throw new HttpsError("unauthenticated", "카카오 인증 코드가 유효하지 않습니다.");
  }

  const tokenData = await tokenResponse.json() as {access_token?: string};
  const accessToken = tokenData.access_token;
  if (!accessToken || accessToken.trim().length === 0) {
    throw new HttpsError("internal", "카카오 액세스 토큰 응답이 비어 있습니다.");
  }
  return accessToken;
}

export const authExchangeKakao = onCall({secrets: [kakaoRestApiKey]}, async (request) => {
...
export const authExchangeKakaoCode = onCall({secrets: [kakaoRestApiKey]}, async (request) => {
  const code = request.data?.code as string | undefined;
  const redirectUri = request.data?.redirectUri as string | undefined;
  ...
  const accessToken = await exchangeKakaoAuthCodeToAccessToken(
    code.trim(),
    redirectUri.trim(),
  );
...
export const seedDemoData = onCall(async (request) => {
  ...
  if (groupId !== "g_demo") {
    throw new HttpsError("permission-denied", "g_demo 그룹에서만 실행할 수 있습니다.");
  }
  if (
    process.env.FUNCTIONS_EMULATOR !== "true" &&
    process.env.ALLOW_DEMO_SEED !== "true"
  ) {
    throw new HttpsError("failed-precondition", "운영 환경에서 데모 시딩이 차단되어 있습니다.");
  }
  ...
  if (!allowed) {
    throw new HttpsError("permission-denied", "운영진 권한이 필요합니다.");
  }
...
type RecomputeSummary = {
  ...
  activityTopCount: number;
};

function nicknameFromMemberData(data: FirebaseFirestore.DocumentData): string | null {
  const publicProfile = data.public;
  if (publicProfile && typeof publicProfile === "object") {
    const nickname = (publicProfile as Record<string, unknown>).nickname;
...
async function recomputeGroupPeriodStatsCore(
  groupId: string,
  periodKey: string,
  ...
  const attendanceScore = new Map<string, number>();
  ...
  for (const eventDoc of eventsSnap.docs) {
    if (eventDoc.data().isDeleted === true) continue;
...
export const recomputeGroupPeriodStats = onCall(async (request) => {
  const callerUid = request.auth?.uid;
  if (!callerUid) {
    throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
  }
  ...
  const groupId = request.data?.groupId as string | undefined;
  const periodKey = request.data?.periodKey as string | undefined;
  if (!groupId || !periodKey) {
    throw new HttpsError("invalid-argument", "groupId와 periodKey가 필요합니다.");
  }
  if (!isValidPeriodKey(periodKey)) {
    throw new HttpsError("invalid-argument", "periodKey 형식은 YYYY-MM 이어야 합니다.");
  }
...
export const onNoticeCreatedPush = onDocumentCreated(
  ...
  async (event) => {
    ...
    const groupName = (groupDoc.data()?.name as string | undefined) ?? "Moyeora";
    const noticeTitle = (notice.title as string | undefined) ?? "새 공지";

    const activeMembers = await getActiveMembers(groupId);
    for (const member of activeMembers) {
      const settings = await getNotificationSettings(groupId, member.uid);
      if (!settings.noticeEnabled) continue;
...
export const onSuggestionCreatedGithubIssue = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (!repo || !token) {
      logger.info("onSuggestionCreatedGithubIssue skipped (missing config)", {
    ...
    try {
      ...
      logger.error("onSuggestionCreatedGithubIssue failed", {
        groupId,
        suggestionId,
        error,
      });
...
export const onBetaReportCreatedGithubIssue = onDocumentCreated(
  "beta_reports/{reportId}",
  async (event) => {
    ...
    const token = (process.env.GITHUB_ISSUES_TOKEN ?? "").trim();
    if (!repo || !token) {
      logger.info("onBetaReportCreatedGithubIssue skipped (missing config)", {
    ...
    try {
      ...
      logger.error("onBetaReportCreatedGithubIssue failed", {
        reportId,
        error,
      });
      await event.data?.ref.set(
...
export const d1EventReminderHourly = onSchedule("every 30 minutes", async () => {
  if (!isAutomationFeatureEnabled("ENABLE_EVENT_REMINDER")) {
    return;
  }

  const reminderHours = parseReminderHours(
    process.env.EVENT_REMINDER_HOURS,
    [24],
  );
  ...
  const upperBound = admin.firestore.Timestamp.fromMillis(
    nowMillis + (maxHours + 1) * 60 * 60 * 1000,
  );
  ...
  for (const groupDoc of groupsSnap.docs) {
    ...
    for (const eventDoc of eventsSnap.docs) {
      ...
      if (eventData.isDeleted === true) continue;

      const startAt = eventData.startAt as FirebaseFirestore.Timestamp | undefined;
      if (!startAt) continue;
      ...
      const pendingHours = reminderHours.filter((hours) =>
        !sentHours.includes(hours) &&
        isWithinReminderWindow(hoursUntilStart, hours, 1)
      );
      if (pendingHours.length === 0) continue;

      const nearestHour = pendingHours[pendingHours.length - 1];
      const eventTitle = (eventData.title as string | undefined) ?? "일정";
...
export const noResponseReminderAt13 = onSchedule(
  "every 30 minutes",
  async () => {
    ...
    const reminderHours = parseReminderHours(
      process.env.NO_RESPONSE_REMINDER_HOURS,
      [24, 6],
    );
    if (reminderHours.length === 0) {
      return;
    ...
    for (const groupDoc of groupsSnap.docs) {
      ...
      for (const eventDoc of eventsSnap.docs) {
        ...
        if (nowMillis >= closeMillis) continue;

        const hoursUntilStart = (startAt.toMillis() - nowMillis) / (60 * 60 * 1000);
        const sentHours = normalizeSentHours(
          eventData.automationNoResponseReminderSentHours,
        );
        const pendingHours = reminderHours.filter((hours) =>
          !sentHours.includes(hours) &&
          isWithinReminderWindow(hoursUntilStart, hours, 1)
        ...
        if (noResponseUids.length === 0) continue;
        const eventTitle = (eventData.title as string | undefined) ?? "일정";
        let sentCount = 0;

        for (const uid of noResponseUids) {
          const settings = await getNotificationSettings(groupId, uid);
          if (!settings.noResponseReminderEnabled) continue;
        ...
        if (sentCount > 0) {
          await eventDoc.ref.set({
            automationNoResponseReminderSentHours:
              admin.firestore.FieldValue.arrayUnion(...pendingHours),
            automationNoResponseReminderUpdatedAt:
...
export const recurringEventAutoCreateEvery30m = onSchedule(
  "every 30 minutes",
  async () => {
    ...
    for (const group of groups) {
      ...
      for (const rootDoc of recurringSnap.docs) {
        try {
          ...
          let createdStartAt: admin.firestore.Timestamp | null = null;
          ...
          await db.runTransaction(async (tx) => {
            ...
            if (!freshRoot.exists) return;

            const data = freshRoot.data() ?? {};
            if (data.isDeleted === true) return;
            if (data.recurrenceEnabled !== true) return;
            ...
            if (!(nextStartRaw instanceof admin.firestore.Timestamp)) return;

            const lookAheadMillis = recurrenceLookAheadMillis(rule);
            if (nextStartRaw.toMillis() > nowMillis + lookAheadMillis) return;
          ...
          logger.error("recurringEventAutoCreateEvery30m failed", {
            groupId,
...
export const feeDueReminderD3D1At10 = onSchedule(
  ...
  async () => {
    ...
    const untilTs = admin.firestore.Timestamp.fromMillis(
    ...
    for (const group of groups) {
      ...
      try {
        ...
        for (const feeDoc of feesSnap.docs) {
          ...
          if (!(dueDateRaw instanceof admin.firestore.Timestamp)) continue;
          const dDay = daysUntilDueInKst(dueDateRaw.toDate(), now);
          if (dDay !== 3 && dDay !== 1) continue;
          ...
          for (const member of activeMembers) {
            ...
            const sentDays = Array.isArray(recordData.reminderSentDays) ?
              recordData.reminderSentDays.filter((v): v is number => typeof v === "number") :
              [];
            if (sentDays.includes(dDay)) continue;

            const settings = await getNotificationSettings(groupId, member.uid);
            if (!settings.paymentReminderEnabled) continue;
        ...
        logger.error("feeDueReminderD3D1At10 failed", {groupId, error});
      }
    }
  },
);

export const eventAutoCloseAndResultNotifyEvery10m = onSchedule(
  "every 10 minutes",
  async () => {
    ...
    for (const group of groups) {
      ...
      try {
        ...
        for (const eventDoc of targetEvents.docs) {
          ...
          try {
            await db.runTransaction(async (tx) => {
              ...
              if (closedByStatus || closedByTime) return;
            ...
            const goingCount = responsesSnap.docs.filter(
              (responseDoc) => responseDoc.data().response === "going",
            ).length;
            const activeMembers = await getActiveMembers(groupId);
            for (const member of activeMembers) {
              const settings = await getNotificationSettings(groupId, member.uid);
            ...
            logger.error("eventAutoCloseAndResultNotifyEvery10m event failed", {
              groupId,
              eventId: eventDoc.id,
              error,
            });
        ...
        logger.error("eventAutoCloseAndResultNotifyEvery10m group failed", {
          groupId,
          error,
        });
      }
...
async function getActiveGroups(): Promise<FirebaseFirestore.QueryDocumentSnapshot[]> {
  const groupsSnap = await admin.firestore().collection("groups").get();
  return groupsSnap.docs.filter((doc) => doc.data().status !== "dormant");
}

async function safeBatchWrite(
  writer: (batch: FirebaseFirestore.WriteBatch) => Promise<void> | void,
): Promise<void> {
  const batch = admin.firestore().batch();
  await writer(batch);
  await batch.commit();
...
export const monthlyFeePeriodAutoCreate = onSchedule(
  {schedule: "5 0 1 * *", timeZone: "Asia/Seoul"},
  async () => {
    ...
    for (const group of groups) {
      ...
      try {
        ...
        logger.error("monthlyFeePeriodAutoCreate failed for group", {groupId, error});
      }
    }
  },
);

export const unpaidMembersReminderAt19 = onSchedule(
    ...
    for (const group of groups) {
      ...
      try {
        ...
        for (const member of activeMembers) {
          ...
          if (!paymentReminderEnabled) continue;
        ...
        logger.error("unpaidMembersReminderAt19 failed for group", {groupId, error});
      }
    }
  },
);

export const leaderboardAndStatsSnapshotAt03 = onSchedule(
  ...
  async () => {
    ...
    for (const group of groups) {
      ...
      try {
        ...
        logger.error("leaderboardAndStatsSnapshotAt03 failed for group", {groupId, error});
      }
    }
  },
);

export const dormantGroupAutoDetectionAt02 = onSchedule(
  ...
  async () => {
    ...
    for (const groupDoc of groupsSnap.docs) {
      ...
      try {
        ...
        const createdAt = groupData.createdAt as FirebaseFirestore.Timestamp | undefined;
        if (!createdAt) continue;

        const elapsedDays = (now - createdAt.toMillis()) / (24 * 60 * 60 * 1000);
        if (elapsedDays < 14) continue;
        ...
        if (activeMembers.length < 5 && groupData.status !== "dormant") {
          ...
          logger.info("Group transitioned to dormant", {groupId});
        }
      } catch (error) {
        logger.error("dormantGroupAutoDetectionAt02 failed for group", {groupId, error});
      }
    }
  },
);
 ...
 */
export const dailyBackupJob = onSchedule(
  {schedule: "0 18 * * *", timeZone: "UTC"},
  async () => {
    // ENABLE_SERVER_FEATURES 조건부 실행
    if (process.env.ENABLE_SERVER_FEATURES === "false") {
      logger.info("dailyBackupJob skipped: ENABLE_SERVER_FEATURES=false");
      return;
    }

    const bucket = (process.env.BACKUP_BUCKET ?? "").trim();
    if (!bucket) {
      logger.error("dailyBackupJob aborted: BACKUP_BUCKET environment variable is not set");
      return;
    }

    const projectId = process.env.GCLOUD_PROJECT ?? admin.instanceId().app.options.projectId;
    if (!projectId) {
      logger.error("dailyBackupJob aborted: could not determine project ID");
      return;
    }
    ...
    try {
      ...
      if (!accessToken) {
        logger.error("dailyBackupJob aborted: failed to obtain access token");
        return;
      }
      ...
      if (!response.ok) {
        ...
        logger.error("dailyBackupJob: Firestore export request failed", {
      ...
      logger.info("dailyBackupJob: Firestore export started", {
        operation: operation.name,
        outputUriPrefix,
      });
    } catch (error) {
      logger.error("dailyBackupJob: unexpected error", {error});
    }
  },
);
